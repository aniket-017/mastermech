...
const Products = () => {
  ...
  const {specifiedStandard, certificationStatus, surfaceFinish, hotForge,coldForge, shade, coatedMaterial, brand, mildSteelGrade, alloySteelGrade} = useSelector((state)=> state.filter.filters)

  useEffect(() => {
    dispatch(getProduct(keyword,currentPage,certificationStatus,surfaceFinish,hotForge,coldForge,shade,coatedMaterial));
  ...
  {venues.map((venue) => (
    <tr key={venue.id}>
      ...
      <td><button to={venue._id}><Link to={venue._id}>Know More!</Link></button> </td>
